...
def isumap(data,
                ...
                for n1 in neighbors:
                    for n2 in neighbors:
                        if n1 != n2:
                            if DC[n1][n2] != 0:
                                tmp = DC[n1][n2]
                                DC[n1][n2] = min(tmp, COEF*data_points[n1])
                            else:
                                DC[n1][n2] = COEF*data_points[n1]
                            if DC[n2][n1] != 0:
                                tmp = DC[n2][n1]
                                DC[n2][n1] = min(tmp, COEF*data_points[n2])
                            else:
                                DC[n2][n1] = COEF*data_points[n2]
            data_D += DC
        ############################################################
        print("\nRunning Dijkstra...")
        t0 = time()
    ...
    # Return the sqrt coefficient used in the algorithm
    sqrt_coefficient = np.sqrt(2) if sqrt else None
    
    return finalEmbedding, clusterLabels, sqrt_coefficient
